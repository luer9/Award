...
<script>
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: "<%=request.getContextPath()%>/award/findCheckAwardinfor",
            type: "POST",
            dataType: "JSON",
                ...
                if (apiResult.code == 1) {
                    apiResult.data.forEach(function (value) {
                        ...
                        $('#tbody').append(
                            ...
                            '<td>' +  '<button class="btn btn-primary" onclick="checkAward('+value.checkId+','+value.awardStuId+','+value.awardId+')">查看</button>'
                            + '</td>' +
                            '</tr>'
                        );
                    });
    ...
    function checkAward(checkId,stuId,awardId){
        alert(checkId + " " + stuId + " " + awardId);
        window.location.href = "AwardCheck.jsp?flag=true&CHECKCODE="+checkId+"&SID="+stuId+"&AID="+awardId;
    }
